...
#    [print(press,end=', ') for press in pressure]
    ...
    ax1.plot(results[:,0], results[:,1], color=color, marker='.', markersize=2.0, linestyle='solid', linewidth=0.5)
    ...
    ax2.plot(results[:,0], results[:,2], color=color, marker='.', markersize=2.0, linestyle='solid', linewidth=0.5)
    ...
    color = 'red'
    plt.ylim(p_min,p_max)
    ax1.plot(results[:,0], results[:,1], color='red')
    ax1.set_xlabel('Time')
    ax1.set_ylabel('Pressure (Pa)', color=color)
    ax1.tick_params(axis='y', labelcolor=color)
    ax1.grid(axis='x')
    ...
    plt.ylim(t_min,t_max)
    ax2.set_ylabel('Temperature (°C)', color=color)
    ax2.plot(results[:,0], results[:,2], color=color)
    ax2.tick_params(axis='y', labelcolor=color)
    fig.suptitle(f'{apg_filename}\nStart: {wndw_start_dt}  -  End: {wndw_end_dt}')
    fig.tight_layout()
    '''
