...
def main():
    ...
    while True:
        ...
        if raw_record:
            ...
            tmptr_period_usec = (int_tempr / (logger.tp_fctr) + logger.tp_cnst) / (
                logger.clock_freq
            )

            # Pressure period (usec)
            presr_period_usec = (int_press / (logger.pp_fctr) + logger.pp_cnst) / (
                logger.clock_freq
            )
            ...
            pressure_psi = coef_cv * facts * (1 - coef_dv * facts)  # pressure in PSIA
