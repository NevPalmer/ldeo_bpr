...
def parse_args_apg_read(args: Sequence[str] | None = None) -> Namespace:
    ...
    parsed_args.plot = {"format": parsed_args.plot[:1], "output": parsed_args.plot[1:]}

    if parsed_args.mseedpath and not (parsed_args.network and parsed_args.station):
        parser.error(
...
def parse_args_apg_read_serial(args: Sequence[str] | None = None) -> Namespace:
...
def infile_parser() -> ArgumentParser:
...
def ini_cfg_parser() -> ArgumentParser:
...
def processing_param_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        "--ignoreticks",
        help="Ignore the time tick values recorded in the raw data file. "
        "Time tick values are generated by assuming that the period between samples "
        "is the precise number of milliseconds for the expected lagging frequency. "
        "That is 10 ms/sample for a 100 Hz logging rate, and 8 ms for a 125 Hz logging "
        "rate.",
        action="store_true",
        default=False,
    )
...
def outfile_parser() -> ArgumentParser:
...
def plot_param_parser() -> ArgumentParser:
...
def serial_param_parser() -> ArgumentParser:
    """Return parser to retrieve plotting parameters."""
    parser = ArgumentParser(add_help=False)
