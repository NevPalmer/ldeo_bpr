...
def main():
    ...
    parser.add_argument("-o","--outfile", 
        help=f'Full path and filename for output file. No file will be generated'
            f'if not specified.',
    ...
    if args.gpssynctime:
        ...
        print(f'Clock drift in milliseconds (logged time minus actual GPS time) = {drift}')


    # Calculate window for extracting data.
    if not args.beginwndw:
    ...
    print('Calculating period of temperature signal.')
    ...
    print('Calculating period of pressure signal.')
    ...
    # Decimate results
    print('Decimating pressure results.')
    ...
    if decmt_intvl > 0:
        ...
        while decmt_intvl_pre > 1:
            ...
            print(f'Decimating with factor = {decmt_fctr}',end='',flush=True)
...
#    [print(press,end=', ') for press in pressure]
    ...
    # Switch the commented lines below to select timestamp as seconds or datetime.
    print('Calculating a timestamp array from the seconds array.')
    if len(pressure_dcmtd) != 0:
        times = seconds_dcmtd
        # times = [clk_start_dt + dt.timedelta(seconds = sec) for sec in seconds_dcmtd]
        pressure_out = pressure_dcmtd
    else:
        times = seconds_p
        # times = [clk_start_dt + dt.timedelta(seconds = sec) for sec in seconds_p]
        pressure_out = pressure
    results = np.column_stack((times, pressure_out, temperature_resmpld))
    ...
    if len(pressure_dcmtd) != 0:
        ax1.plot(seconds_p, pressure, color='pink', marker='.', markersize=1.0, linestyle='')
    ax1.plot(results[:,0], results[:,1], color=color, marker='.', markersize=2.0, linestyle='', linewidth=0.5)
    ...
    if len(pressure_dcmtd) != 0:
        ax2.plot(seconds_t, temperature, color='lightblue',marker='.', markersize=1.0, linestyle='')
    ax2.plot(results[:,0], results[:,2], color=color, marker='.', markersize=2.0, linestyle='', linewidth=0.5)
    ax2.tick_params(axis='y', labelcolor=color)
    fig.suptitle(f'{apg_filename}\nStart: {wndw_start_dt}  -  End: {wndw_end_dt}')
    fig.tight_layout()
