...
# 2020/08/11 Minor edits for progress outputs and plotting.
...
#    [print(press,end=', ') for press in pressure]
    ...
    # Calculating datetime array from seconds array.
    '''
    print('Calculating datetime array from seconds array.')
    ...
    #Plot with seconds on X axis
    
    fig, ax1 = plt.subplots()
    color = 'red'
    plt.ylim(p_min,p_max)
    ax1.plot(seconds_p, pressure, color='red',marker='.',markersize=1.0,linestyle='')
    if len(pressure_dcmtd) != 0:
    ...
    ax2.plot(seconds_t, temperature, color=color,marker='.',markersize=1.0,linestyle='')
    if len(pressure_dcmtd) != 0:
