...
def main():
    ...
    etech_turnTime = 12.5 # Delay in ms for reply from BPR transducer.
    etech_sndSpd = 1500 # Speed of sound in water (typical 1450 to 1570 m/sec)
    ...
    with serial.Serial(port=etech_ser_port,
        ...
        while True:
            ...
            responseLine, flag = getResponse(ser)
            if responseLine != b'':
                now = dt.datetime.now()
                with open('./raw_response.txt', 'a+') as rawfile:
                    rawfile.write(f'{now}: {responseLine}\n')
            
                responseItem = responseLine.decode('UTF-8').strip().split(' ')
                
                if (responseItem[0] == 'RNG:'):
                    #print('==================================================================')
                    ...
                    try:
                        ...
                        range['tx'] = float(responseItem[3])
                        range['rx'] = float(responseItem[6])
                        range['rangeTime'] = float(responseItem[9])
                        range['range'] = (range['rangeTime'] /2) * range['sndSpd']
                    except:
                        print(f'Serial range response string was incomplete. No range has been logged.')
                        print(f'String received: {responseLine}')
                        continue
