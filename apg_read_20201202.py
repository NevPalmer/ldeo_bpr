...
def main():
    ...
    file_duration_days = nrecs*smpls_per_rec*sample_epoch/3600000/24
    ...
    rec_begin = int(wndw_begin_secs*1000 / sample_epoch / smpls_per_rec)
    print(f'First record of data window: {rec_begin}')
    nrecs_want = int(wndw_len_secs*1000 / sample_epoch / smpls_per_rec)+1
    ...
    nominal_begin_tick = rec_begin * sample_epoch * smpls_per_rec
    nominal_end_tick_plus1 = (rec_begin + len(ticks)) * sample_epoch * smpls_per_rec
    ...
    for idx, rollover in np.ndenumerate(rollovers):
        ...
        if nxt_rollover - rollover == 1:
            ticks[rollover] = ticks[rollover-1] + sample_epoch * smpls_per_rec
        else:
    ...
    millisecs_t = np.linspace(nominal_begin_tick, nominal_end_tick_plus1,
                            num=len(ticks),
                            endpoint=False)
    millisecs_p = np.linspace(nominal_begin_tick, nominal_end_tick_plus1,
                            num=len(ticks)*smpls_per_rec,
                            endpoint=False)
    nominal_end_tick = millisecs_p[-1]
