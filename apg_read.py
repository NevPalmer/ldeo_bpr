...
def main():
    ...
    parser.add_argument("-n", "--station",
                        help='Station name to be used in MiniSEED file '
                        'header. Max 5 characters.',
                        default="")
    ...
    logger['smpls_per_rec'] = logger_cfg.getint(
                                    logger_version, 'smpls_per_rec')
    ...
    logger['TP_fctr'] = evaluate(logger_cfg.get(
                                    logger_version, 'TP_fctr')).item()
    logger['TP_cnst'] = logger_cfg.getfloat(logger_version, 'TP_cnst')
    logger['PP_fctr'] = evaluate(logger_cfg.get(
                                    logger_version, 'PP_fctr')).item()
    ...
    if abs(fmt_field['pcore']-fmt_field['pn']+1) != logger['smpls_per_rec']:
        sys.exit(f"The number of samples per record "
    ...
    if args.beginwndw is None:
        ...
        wndw_begin_dt = dt.datetime.strptime(
                                        wndw_begin, '%Y_%m_%d_%H_%M_%S.%f')
...
def generate_results(
    ...
    if (actual_begin_tick == nominal_begin_tick
        ...
        final_end_tick = (actual_end_tick
                          - (actual_end_tick % logger['record_epoch']))
        epoch_count = int((final_end_tick - final_begin_tick)
                          / logger['record_epoch']) + 1
        ...
        nom_ticks_t = np.append(nom_ticks_t,
                                nom_ticks_t[-1]+logger['record_epoch'])
    ...
    drift_applied = int(
        logger['sample_epoch'] *
        round(drift_applied / logger['sample_epoch'], 0)
    )
    ...
    TP = (temp_raw / (logger['TP_fctr'])
          + logger['TP_cnst']) / (logger['clock_freq'])
    ...
    PP = (press_raw / (logger['PP_fctr'])
          + logger['PP_cnst']) / (logger['clock_freq'])
    ...
    if mseed_path:
        # Trim results to the originally specified time bin.
        bin_end_ms = bin_begin_ms + bin_len_ms
        mask = np.logical_and(millisecs_p >= bin_begin_ms,
                              millisecs_p < bin_end_ms)
...
###########################################################################
def extractrecords(apg_filename, logger, nrecs_want, rec_begin, trbl_sht,
                   clk_start_dt):
    '''
    ...
    with open(apg_filename, 'rb') as apgfile:
        ...
        if logger['timing'] == 'first':
            ...
            first_tic = int((logger['smpls_per_rec'] - 1)
                            * logger['sample_epoch'])
        last_field = len(logger['rec_fmt']) - 1
        # ticks are equivalent to milliseconds
        ticks = (records[:, last_field - logger['fmt_field']['tic']]
                 - first_tic)
        ...
        for idx, rollover in np.ndenumerate(rollovers):
            ...
            if (ticks[rollover+1] - ticks[rollover]) != 0:
                ...
                if nxt_rollover - rollover == 1:
                    ...
                    ticks[rollover] = (ticks[rollover-1]
                                       + logger['record_epoch'])
                elif (
                    ...
                    ticks[rollover:nrecs_want] = (ticks[rollover:nrecs_want]
                                                  + rollover_period)
                    rollover_dt = (clk_start_dt
                                   + np.timedelta64(ticks[rollover], "ms"))
                    print(
...
def clockdrift(
    ...
    if sync_tick_count is None:
        ...
        sync_records = extractrecords(apg_filename, logger, nrecs_want,
                                      rec_begin, trbl_sht, clk_start_dt)
